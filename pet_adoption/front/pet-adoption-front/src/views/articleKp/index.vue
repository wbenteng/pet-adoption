<template>
  ...
  <el-card style="margin: 10px 0;">
    <el-button type="primary" size="default" @click="addPet">添加科普文章</el-button>
    ...
    <el-table style="margin: 10px 0;" border stripe :data="tableData" @selection-change="selectChange">
      ...
      <el-table-column align="center" prop="time" label="发表时间"></el-table-column>
      ...
      <el-table-column align="center" prop="read1" label="阅读数"></el-table-column>
  ...
  <!-- 对话框组件:添加|更新科普文章 -->
  <el-dialog v-model="dialogFormVisible" width="60%" :title="articleParams.id ? '修改科普文章' : '添加科普文章'"
    ...
    <el-form style="width: 100%" ref="formRef">
      ...
      <el-form-item label="封面">
        <el-upload class="avatar-uploader" action="api/file/upload" :show-file-list="false"
          ...
          <img v-if="articleParams.img" :src="articleParams.img" class="avatar" />
...
<script setup lang="ts" name="ArticleKp">
...
let content = ref('')
//收集科普文章信息的数据
let articleParams = reactive<any>({
  id: '',
  name: '',
  content: '',
  img: ''
...
const delBatch = async () => {
  ...
  let result: any = await reqDeleteBatcbArticleKp(idsList)
  // console.log(result);
  if (result.code == 200) {
    ElMessage({
      type: 'success',
