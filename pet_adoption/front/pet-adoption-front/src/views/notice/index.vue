<template>
  ...
  <el-card style="margin: 10px 0;">
    <el-button type="primary" size="default" @click="addPet">添加公告</el-button>
    ...
    <el-table style="margin: 10px 0;" border stripe :data="tableData" @selection-change="selectChange">
      ...
      <el-table-column align="center" label="操作" width="170px">
        <template #="{ row, $index }">
          <el-button type="primary" size="small" icon="Edit" @click="updateNotice(row)">编辑</el-button>
    ...
    <el-pagination v-model:current-page="currentPage" v-model:page-size="pageSize" :page-sizes="[3, 5, 7, 10]"
      ...
      @current-change="getHasNotice" @size-change="handler" />
  </el-card>
  <!-- 对话框组件:添加|更新公告 -->
  <el-dialog v-model="dialogFormVisible" :title="noticeParams.id ? '修改公告' : '添加公告'" :close-on-click-modal="false">
    <el-form label-width="120px" style="width: 80%" ref="formRef">
      <el-form-item label="名称">
        <el-input v-model="noticeParams.name" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="内容">
        <el-input v-model="noticeParams.content" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="时间">
        <el-date-picker v-model="noticeParams.time" type="datetime" value-format="YYYY-MM-DD HH:mm:ss"
...
import { reqSearchNotice, reqAddNotice, reqDeleteNotice, reqDeleteBatcbNotice } from '@/api/notice';
...
//获取全部已有公告信息
const getHasNotice = async (pager = 1) => {
  currentPage.value = pager
  let result = await reqSearchNotice({
    params: {
...
const save = async () => {
  ...
  //发请求
  let result: any = await reqAddNotice(noticeParams)
  // console.log(result);
  if (result.code == 200) {
    dialogFormVisible.value = false
    ElMessage({
      type: 'success',
      message: noticeParams.id ? '更新成功' : '添加成功'
    })
    //获取全部公告
    getHasNotice(noticeParams.id ? currentPage.value : 1)
    //浏览器刷新
    ...
    ElMessage({
      type: 'error',
      message: noticeParams.id ? '更新失败' : '添加失败'
    })
  }
}
//删除某一个公告
const deleteNotice = async (petId: number) => {
  let result: any = await reqDeleteNotice(petId)
  if (result.code == 200) {
    ElMessage({
      type: 'success',
      message: '删除成功'
    })
    getHasNotice(tableData.value.length > 1 ? currentPage.value : currentPage.value - 1)
  }
}
//table复选框勾选触发
...
const delBatch = async () => {
  ...
  let result: any = await reqDeleteBatcbNotice(idsList)
  // console.log(result);
  if (result.code == 200) {
    ElMessage({
      type: 'success',
